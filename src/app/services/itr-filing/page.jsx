    ...
    try {
      const res = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          ...
          subject: "ITR Filing Enquiry - ZAMEXO",
        }),
      });
      ...
      if (res.ok && data.success) {
        setSubmitMessage("✅ Thank you! Our tax expert will contact you shortly.");
        ...
        // Success message 5 seconds के बाद clear करें
        setTimeout(() => {
          setSubmitMessage("");
        }, 5000);
      } else {
  ...
  return (
    <main className="bg-white">
          ...
          <div className="bg-gradient-to-br from-[#0B1C2D] to-blue-900 text-white rounded-3xl p-10 shadow-2xl">
            ...
            <ul className="space-y-4 text-blue-100">
              <li className="flex items-center gap-2">
                <AlertTriangle size={18} /> Penalties & interest on late filing
              </li>
              <li className="flex items-center gap-2">
                <AlertTriangle size={18} /> Legal scrutiny by Income Tax Department
              </li>
              <li className="flex items-center gap-2">
                <AlertTriangle size={18} /> Delay in refund processing
              </li>
              <li className="flex items-center gap-2">
                <AlertTriangle size={18} /> Difficulty in availing loans
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div
                key={i}
                className="group bg-white border rounded-2xl p-8 shadow hover:-translate-y-2 hover:shadow-xl hover:border-[#D4AF37] transition duration-300"
              >
                <Users className="text-[#D4AF37] mb-4" size={32} />
                <h4 className="font-semibold text-lg mb-2 text-[#0B1C2D] group-hover:text-[#D4AF37]">
                  {item[0]}
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-4 gap-6">
          {[
            ...
            <div
              key={i}
              className="relative bg-white p-6 rounded-2xl border shadow hover:shadow-lg transition"
            >
              <span className="absolute -top-4 left-6 bg-[#D4AF37] text-[#0B1C2D] px-3 py-1 rounded-full text-sm font-bold">
                Step {i + 1}
              </span>
      ...
      <section className="max-w-4xl mx-auto px-6 py-24">
        ...
        <form
          onSubmit={handleSubmit}
          className="bg-white border border-gray-200 rounded-2xl p-8 md:p-10 shadow-xl grid gap-5 text-black"
        >
          {[
            ["name", "Full Name *", "text"],
            ...
            <div key={i} className="grid gap-1">
              ...
              <input
                ...
                value={formData[f[0]]}
                onChange={handleChange}
                placeholder={`Enter ${f[1].toLowerCase().replace(' *', '')}`}
                className="border border-gray-300 p-3.5 rounded-lg focus:ring-2 focus:ring-[#D4AF37]/50 focus:border-[#D4AF37] outline-none transition"
                required
                type={f[2]}
              />
            </div>
          ...
          <div className="grid gap-1">
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="Any specific tax query or requirement?"
              rows={4}
              className="border border-gray-300 p-3.5 rounded-lg focus:ring-2 focus:ring-[#D4AF37]/50 focus:border-[#D4AF37] outline-none transition"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-4 bg-[#D4AF37] text-[#0B1C2D] py-3.5 rounded-xl font-semibold hover:bg-[#c9a633] transition flex items-center justify-center gap-2 shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-[#0B1C2D]"></div>
                Processing...
              </>
            ) : (
              <>
          ...
          <p className="text-xs text-center text-gray-500 mt-2">
            🔒 Your data is fully confidential and secure. We respect your privacy.
          </p>
        </form>
      </section>
